...
//		emit Deposit ( _erc721 , _tokenid );
	...
	function deposit_mint ( address _erc721, uint256 _tokenid ) public {
		if(!existenceCheck(_tokenid)){
			IKIP17(_erc721).mint(msg.sender, _tokenid);
		} else {
			require(IKIP17(_erc721).ownerOf(_tokenid) == msg.sender, "ERR():: Not an owner");
		}
		IKIP17 ( _erc721).safeTransferFrom ( msg.sender , address(this) , _tokenid , "0x00") ;		
